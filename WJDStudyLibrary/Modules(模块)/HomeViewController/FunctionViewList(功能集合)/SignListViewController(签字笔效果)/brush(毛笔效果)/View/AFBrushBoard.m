...
- (void)updateUI {
   ...
   // self.backgroundColor = [UIColor colorWithPatternImage:JDGetImage(@"background")];
    ...
    UIButton *cleanBtn = [JDUIFactory createButtonWithFrame:CGRectMake(SCREEN_WIDHT/2,self.frame.size.height-50, self.frame.size.width/2, 50) ImageName:nil Target:self Action:@selector(cleanBtnDidClick) Title:@"清除"];
...
-(BOOL)saveImage
{
    CGRect rect =  CGRectMake(_mixPoint.x-kWIDTH_MAX, _mixPoint.y-kWIDTH_MAX, _maxPoint.x-_mixPoint.x+kWIDTH_MAX*2, _maxPoint.y-_mixPoint.y+kWIDTH_MAX*2);
    
    CGImageRef imageRef=self.lastImage.CGImage;
    CGImageRef imagePartRef=CGImageCreateWithImageInRect(imageRef,rect);
    UIImage*cropImage=[UIImage imageWithCGImage:imagePartRef];
    CGImageRelease(imagePartRef);;
    
    NSString *path =kPathTem;
...
- (void)changeImage {
    ...
    // 目标半径
    CGFloat aimWidth = 150.0/(CGFloat)len * (kWIDTH_MAX - kWIDTH_MIN);
    // 获取贝塞尔点集
